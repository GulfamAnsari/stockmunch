  ...
  return (
    ...
    >
      ...
      {isWatchlist && news.userSentiment && (
        <div
          className={`absolute -top-2 -left-2 px-2 py-0.5 rounded text-[8px] font-black uppercase tracking-widest z-30 border shadow-lg ${
            news.userSentiment === "BULLISH"
      ...
      <div className="p-4 flex flex-col flex-grow">
        <div className="flex items-start justify-between mb-3 gap-2">
          <div className="flex items-center space-x-3 min-w-0">
            <div
              className={`w-9 h-9 shrink-0 rounded-lg flex items-center justify-center text-white text-[11px] font-black shadow-inner overflow-hidden border border-white/10`}
            >
              {news.logoUrl ? (
                <img
                  src={news.logoUrl}
        ...
        <div className="mt-auto">
          {news.aiAnalysis && (
            <div className="mb-3 p-3 bg-white/5 rounded-lg border border-white/10">
              <span className="text-[8.5px] font-black text-emerald-500 uppercase tracking-widest block mb-1">
                AI Deep Analysis
              </span>
          ...
          <div className="flex items-center gap-2 mb-4">
            <div
              className={`px-2 py-1.5 rounded-lg border text-[9px] font-black uppercase tracking-widest inline-flex items-center ${getSentimentStyles(
                news.sentiment
              )}`}
            >
              <div
        ...
        <div className="pt-4 flex items-center justify-between border-t border-white/5 gap-2">
          <div className="flex items-center gap-1.5 relative" ref={dropdownRef}>
            <button
              onClick={(e) => {
                e.stopPropagation();
                setShowWatchlistOpts(!showWatchlistOpts);
              }}
            ...
            {showWatchlistOpts && (
              <div className="absolute bottom-full left-0 mb-2 w-32 bg-[#1c2230] border border-white/10 rounded-xl overflow-hidden shadow-2xl z-50 animate-in fade-in slide-in-from-bottom-2">
                <button
                  onClick={(e) => {
                    e.stopPropagation();
                    onWatchlistAdd({ ...news, userSentiment: "BULLISH" });
                    setShowWatchlistOpts(false);
                  }}
                ...
                <button
                  onClick={(e) => {
                    e.stopPropagation();
                    onWatchlistAdd({ ...news, userSentiment: "BEARISH" });
                    setShowWatchlistOpts(false);
                  }}
...
const MarketTerminal: React.FC = () => {
        ...
        Object.keys(json.data).forEach((dateKey) => {
          ...
          const mappedItems: StockNews[] = rawItems.map((item: any) => ({
            ...
            aiAnalysis:
              item.summary ||
              item.data.summary ||
              item.machineLearningSentiments?.explanation,
            ...
            priceChange: 0,
            sentiment:
              item.machineLearningSentiments?.label === "negative"
                ? "bearish"
                : item.machineLearningSentiments?.label === "positive"
                ? "bullish"
                : "neutral",
            sentimentScore: Math.round(
              (item.machineLearningSentiments?.confidence || 0.5) * 100
            ),
            from: item.from,
  ...
  const processedNews = useMemo(() => {
    ...
    if (!sentimentFilters.includes("ALL")) {
      list = list.filter((n) =>
        sentimentFilters.includes(n.sentiment.toUpperCase())
      );
    }
  ...
  const pagedNews = useMemo(
    () => processedNews.slice(0, displayLimit),
    [processedNews, displayLimit]
  );

  const copyAllTitles = () => {
    const content = processedNews
      .map((n) => `${n.timestamp} | ${n.symbol || n.bseCode} | ${n.title}`)
      .join("\n");
                    ...
                    {["ALL", "BULLISH", "BEARISH", "NEUTRAL"].map((opt) => (
                      ...
                      >
                        <div className="relative flex items-center">
                          ...
                          />
                          <svg
                            className="absolute w-3 h-3 text-slate-950 left-0.5 pointer-events-none hidden peer-checked:block"
                            fill="none"
                            viewBox="0 0 24 24"
                            stroke="currentColor"
                            strokeWidth="4"
