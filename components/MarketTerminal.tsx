  ...
  return (
    <div
      ...
      className={`bg-[#111621] border ${
        shouldHighlight ? "border-emerald-500 shadow-[0_0_15px_rgba(16,185,129,0.3)]" : "border-white/5"
    ...
    >
      ...
      {isWatchlist && news.userSentiment && (
        <div className={`absolute -top-2 -left-2 px-2 py-0.5 rounded text-[8px] font-black uppercase tracking-widest z-30 border shadow-lg ${
            news.userSentiment === "BULLISH" ? "bg-emerald-500 text-slate-950 border-emerald-400" : "bg-rose-500 text-white border-rose-400"
          }`}>
          {news.userSentiment}
        </div>
      )}
      ...
      <div className="p-4 flex flex-col flex-grow">
        <div className="flex items-start justify-between mb-3 gap-2">
          <div className="flex items-center space-x-3 min-w-0">
            <div className={`w-9 h-9 shrink-0 rounded-lg flex items-center justify-center text-white text-[11px] font-black shadow-inner overflow-hidden border border-white/10`}>
              {news.logoUrl ? (
            ...
            <div className="min-w-0">
              <div className="flex flex-wrap items-center gap-x-2">
                <h3 className="text-[11px] font-black text-blue-400 tracking-tight uppercase leading-none truncate max-w-[120px]">
                  {news.companyName}
                </h3>
          ...
          <div className="text-right shrink-0">
            <p className="text-[9.5px] text-slate-200 font-mono font-bold uppercase tracking-tight leading-none mb-1">
        ...
        <div className="mt-auto">
          ...
          <div className="flex items-center gap-2 mb-4">
            ...
            <div className="bg-white/5 border border-white/10 px-2 py-1.5 rounded-lg text-[9px] font-mono text-slate-200 uppercase tracking-tight whitespace-nowrap">
              Confidence: <span className="text-white font-bold">{news.sentimentScore}%</span>
            </div>
          </div>
        </div>
        ...
        <div className="pt-4 flex items-center justify-between border-t border-white/5 gap-2">
          <div className="flex items-center gap-1.5 relative" ref={dropdownRef}>
            ...
            <button onClick={(e) => { e.stopPropagation(); fetchLocalPercent(true); }} className="p-1.5 bg-white/5 hover:bg-sky-500/10 text-slate-300 hover:text-sky-500 rounded-lg border border-white/10 transition-all" title="Refresh price">
          ...
          <div className="flex items-center space-x-2">
            <span className="text-[9px] text-slate-400 font-black uppercase tracking-widest">{news.source}</span>
          </div>
        </div>
      </div>
    </div>
  ...
  return (
    <div className="flex-grow flex flex-col min-h-0 bg-[#0b0f1a] overflow-x-hidden relative">
      <div className="lg:hidden shrink-0 bg-[#0d121f] px-4 py-2 flex items-center justify-between border-b border-white/5">
        ...
        <button onClick={() => setIsControlsVisible(!isControlsVisible)} className="px-3 py-1 bg-white/5 border border-white/10 rounded-lg text-[9px] font-black uppercase tracking-widest text-emerald-500 hover:bg-emerald-500/10 transition-all flex items-center space-x-1">
      ...
      <div className={`${isControlsVisible ? 'flex' : 'hidden lg:flex'} px-4 md:px-8 py-3 shrink-0 bg-[#0d121f] border-b border-white/5 flex flex-col lg:flex-row items-stretch lg:items-center justify-between gap-y-4 gap-x-6`}>
        <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-4 flex-wrap">
          <div className="flex bg-slate-900/60 rounded-xl p-1 border border-white/5 shadow-inner shrink-0 self-start sm:self-auto">
            {["ALL FEEDS", "WATCHLIST"].map((tab) => (
              <button key={tab} onClick={() => { setActiveTab(tab); setIsFilterPanelOpen(false); }} className={`px-4 py-2 text-[9px] font-black uppercase tracking-widest rounded-lg transition-all ${ activeTab === tab ? "bg-emerald-600 text-slate-950 shadow-lg" : "text-slate-500 hover:text-white" }`}>{tab}</button>
            ))}
          </div>
          <div className="relative w-full sm:w-48 lg:w-72 shrink-0">
            ...
            <input type="text" placeholder="SEARCH SYMBOL..." value={searchTerm} onChange={(e) => { setSearchTerm(e.target.value); }} className="w-full bg-slate-900 border border-white/10 rounded-lg pl-10 pr-3 py-2 text-[10px] text-white focus:outline-none focus:border-emerald-500 transition-all font-mono placeholder:text-slate-500/70" />
          </div>
          {activeTab !== "WATCHLIST" && (
            <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 shrink-0">
              <div className="flex items-center space-x-2 bg-slate-900/50 p-1 rounded-lg border border-white/10">
                <style dangerouslySetInnerHTML={{ __html: `input[type="date"]::-webkit-calendar-picker-indicator { filter: invert(1) opacity(0.7); cursor: pointer; }` }} />
              ...
              <button onClick={() => setAutoRefresh(!autoRefresh)} className={`px-4 py-2 rounded-lg text-[9px] font-black uppercase tracking-widest border transition-all flex items-center justify-center space-x-2 sm:w-auto w-full ${ autoRefresh ? "bg-emerald-500/10 border-emerald-500 text-emerald-400" : "bg-slate-900 border-white/10 text-slate-400 hover:text-white" }`}>
        ...
        <div className="flex items-center justify-between sm:justify-end gap-3 shrink-0 relative">
          <div className="flex items-center gap-3">
            ...
            <button onClick={copyAllTitles} className="p-2 bg-slate-900 hover:bg-slate-800 text-slate-400 hover:text-white rounded-lg border border-white/10 transition-all flex items-center justify-center" title="Copy Current Dispatch"><svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 7v8a2 2 0 002 2h6M8 7V5a2 2 0 012-2h4.586a1 1 0 01.707.293l4.414 4.414a1 1 0 01.293.707V15a2 2 0 01-2 2h-2M8 7H6a2 2 0 00-2 2v10a2 2 0 002 2h8a2 2 0 002-2v-2" /></svg></button>
            <button onClick={toggleFullScreen} className={`p-2 bg-slate-900 hover:bg-slate-800 rounded-lg border border-white/10 transition-all flex items-center justify-center ${ isFullScreen ? "text-emerald-500 border-emerald-500/50" : "text-slate-400 hover:text-white" }`} title={isFullScreen ? "Exit Full Screen" : "Enter Full Screen"}><svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 8V4m0 0h4M4 4l5 5m11-1V4m0 0h-4m4 0l-5 5M4 16v4m0 0h4m-4 0l5-5m11 5v-4m0 4h-4m4 0l-5-5" /></svg></button>
          </div>
          {isFilterPanelOpen && (
            <div ref={filterPanelRef} className={`absolute top-full mt-3 w-72 bg-[#161b27] border border-white/10 rounded-2xl shadow-2xl p-6 z-[100] animate-in fade-in zoom-in-95 duration-200 ${ filterDropdownSide === "left" ? "right-0" : "left-0" }`}>
                    ...
                    {["ALL", "BULLISH", "BEARISH", "NEUTRAL"].map((opt) => (
                      <label key={opt} className="flex items-center space-x-3 cursor-pointer group">
                        <div className="relative flex items-center">
                          <input type="checkbox" checked={sentimentFilters.includes(opt)} onChange={() => handleSentimentToggle(opt)} className="peer h-4 w-4 appearance-none border border-white/20 rounded bg-slate-900 checked:bg-emerald-500 checked:border-emerald-500 transition-all cursor-pointer" />
                        ...
                        <span className={`text-[10px] font-black uppercase tracking-widest transition-colors ${ sentimentFilters.includes(opt) ? "text-white" : "text-slate-500 group-hover:text-slate-300" }`}>{opt}</span>
                      </label>
                    ))}
                  </div>
                </div>
      ...
      <div ref={scrollContainerRef} onScroll={handleScroll} className="flex-grow overflow-y-auto px-4 md:px-8 py-8 custom-scrollbar bg-black/5 overflow-x-hidden">
        {loading && news.length === 0 ? (
          <div className="h-full flex flex-col items-center justify-center space-y-6"><div className="w-16 h-16 border-4 border-emerald-500 border-t-transparent rounded-full animate-spin"></div><p className="text-[12px] font-black uppercase tracking-[0.4em] opacity-30 text-center">Connecting Stream...</p></div>
      ...
      <footer className="shrink-0 bg-[#111621] border-t border-white/5 px-4 md:px-8 py-3 flex flex-col sm:flex-row items-center justify-between text-[9px] font-black font-mono text-slate-600 tracking-[0.2em] uppercase gap-2">
        <div className="flex items-center space-x-6 md:space-x-10"><div className="flex items-center space-x-2"><span className="text-emerald-500 font-black">SYSTEM:</span><span>READY</span></div><div className="flex items-center space-x-2"><span className="text-emerald-500 font-black">STREAM:</span><span>{processedNews.length} SYNCED</span></div></div>
        <div className="flex items-center space-x-2"><div className="w-1.5 h-1.5 rounded-full bg-emerald-500 opacity-40"></div><span className="opacity-60 italic tracking-tight uppercase text-center">StockManch</span></div>
      </footer>
    </div>
  );
};
