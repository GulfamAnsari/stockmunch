...
const MarketTerminal: React.FC<{ 
  ...
  return (
    <div className="flex-grow flex flex-col min-h-0 bg-[#0b0f1a] overflow-x-hidden relative">
      {/* Redesigned Header with improved visibility */}
      <div className={`${isControlsVisible ? 'flex' : 'hidden lg:flex'} px-4 md:px-6 py-4 shrink-0 bg-[#0d121f] border-b border-white/[0.08] flex flex-col lg:flex-row items-stretch lg:items-center justify-between gap-y-4 gap-x-6 z-40`}>
        <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-4 flex-wrap flex-grow">
          
          {/* Navigation Tabs */}
          <div className="flex bg-slate-950 rounded-2xl p-1 border border-white/[0.1] shadow-xl shrink-0 self-start sm:self-auto">
            {["ALL FEEDS", "BSE FEEDS", "WATCHLIST"].map((tab) => (
              <button 
                key={tab} 
                onClick={() => { setActiveTab(tab); setIsFilterPanelOpen(false); }} 
                className={`px-5 py-2.5 text-[10px] font-black uppercase tracking-[0.15em] rounded-xl transition-all ${ activeTab === tab ? "bg-blue-600 text-white shadow-lg shadow-blue-900/40" : "text-slate-500 hover:text-slate-300" }`}
              >
                {tab}
              </button>
            ))}
          ...
          {activeTab === "BSE FEEDS" ? (
             <div className="flex items-center space-x-3 flex-wrap gap-y-2 animate-in fade-in slide-in-from-left-2 duration-300 flex-grow">
                <div className="flex items-center space-x-3 bg-slate-900/80 px-4 py-2.5 rounded-xl border border-white/[0.1] shadow-inner">
                  ...
                  <select 
                    ...
                    className="bg-transparent border-none text-[10px] font-black uppercase tracking-tight text-slate-200 focus:outline-none cursor-pointer"
                  >
                ...
                <div className="relative w-full sm:w-48 lg:w-64 shrink-0">
                  ...
                  <input 
                    ...
                    className="w-full bg-slate-900/60 border border-white/[0.15] rounded-xl pl-12 pr-4 py-3 text-[11px] text-white focus:outline-none focus:border-blue-500/40 transition-all font-mono placeholder:text-slate-500" 
                  />
                </div>
                <button 
                    onClick={() => setBseAwardsOnly(!bseAwardsOnly)}
                    className={`px-4 py-3 rounded-xl text-[9px] font-black uppercase tracking-widest border transition-all flex items-center space-x-2 ${ bseAwardsOnly ? "bg-amber-500/20 border-amber-500/50 text-amber-500 shadow-lg" : "bg-slate-900/40 border-white/[0.1] text-slate-500 hover:text-slate-300" }`}
                >
                    <span className="hidden xl:inline">Order_Receipt</span>
                </button>
                <button 
                  onClick={() => setBseAutoRefresh(!bseAutoRefresh)} 
                  className={`px-4 py-3 rounded-xl text-[9px] font-black uppercase tracking-widest border transition-all flex items-center justify-center space-x-2 ${ bseAutoRefresh ? "bg-emerald-600/10 border-emerald-600/50 text-emerald-500" : "bg-slate-900/40 border-white/[0.1] text-slate-500 hover:text-slate-300" }`}
                >
            ...
            <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-4 flex-grow">
              ...
              {activeTab === "ALL FEEDS" && (
                <>
                  <div className="flex items-center space-x-2 bg-slate-900/80 p-1.5 rounded-xl border border-white/[0.1] shadow-inner">
                    <input type="date" value={fromDateInput} onChange={(e) => setFromDateInput(e.target.value)} className="bg-slate-950/50 border border-white/5 rounded-lg px-3 py-1.5 text-[10px] text-slate-400 font-mono focus:border-blue-500/40 focus:outline-none w-full sm:w-[125px] cursor-pointer" />
                    <span className="text-slate-700 text-[10px]">→</span>
                    <input type="date" value={toDateInput} onChange={(e) => setToDateInput(e.target.value)} className="bg-slate-950/50 border border-white/5 rounded-lg px-3 py-1.5 text-[10px] text-slate-400 font-mono focus:border-blue-500/40 focus:outline-none w-full sm:w-[125px] cursor-pointer" />
                  ...
                  <button onClick={() => setAutoRefresh(!autoRefresh)} className={`px-5 py-3 rounded-xl text-[10px] font-black uppercase tracking-widest border transition-all flex items-center justify-center space-x-2 ${ autoRefresh ? "bg-emerald-600/10 border-emerald-600/50 text-emerald-500" : "bg-slate-900/40 border-white/[0.1] text-slate-500 hover:text-slate-300" }`}>
        ...
        <div className="flex items-center gap-3 shrink-0 relative">
          <button ref={filterBtnRef} onClick={(e) => { e.stopPropagation(); setIsFilterPanelOpen(!isFilterPanelOpen); }} className={`px-4 py-3 rounded-xl text-[10px] font-black uppercase tracking-widest border transition-all flex items-center space-x-2 ${ isFilterPanelOpen ? "bg-blue-600 text-white border-blue-600 shadow-lg" : "bg-slate-900/40 border-white/[0.1] text-slate-500 hover:text-slate-300" }`}>
