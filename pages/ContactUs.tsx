...
const ContactUs: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!emailRegex.test(formState.email)) {
      setError("Please provide a valid email protocol.");
      return;
    }
    if (formState.subject.trim().length < 3) {
      setError("Subject must be detailed (min 3 chars).");
      return;
    }
    if (formState.message.trim().length < 10) {
      setError("Message body too short (min 10 chars).");
      return;
    }
    ...
    try {
      ...
      setError(err.message || "Network transmission failure. Retry later.");
    } finally {
      setLoading(false);
    }
  };
      ...
      <main className="pt-32 pb-24 px-6">
        <div className="max-w-6xl mx-auto">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-start">
            ...
            {/* Form Side */}
            <div className="bg-[#161b27]/60 border border-white/5 p-10 md:p-14 rounded-[3rem] backdrop-blur-md shadow-2xl relative overflow-hidden">
              {submitted && (
                <div className="absolute inset-0 z-20 bg-emerald-500/10 backdrop-blur-xl rounded-[3rem] flex items-center justify-center p-8 text-center animate-in fade-in zoom-in">
                  <div>
                    <div className="w-20 h-20 bg-emerald-500 text-slate-900 rounded-full flex items-center justify-center mx-auto mb-6 text-3xl font-black shadow-2xl">✓</div>
                    <h3 className="text-3xl font-black text-white mb-2 uppercase tracking-tighter">Inquiry Logged</h3>
                    <p className="text-slate-300 font-medium mb-8">Our node will respond within 24 standard terminal hours.</p>
                    <button 
              ...
              <form onSubmit={handleSubmit} className="space-y-8 relative z-10">
                {error && (
                  <div className="p-4 bg-rose-500/10 border border-rose-500/20 rounded-2xl flex items-center space-x-3 animate-shake">
                    ...
                    <span className="text-rose-500 text-[10px] font-black uppercase tracking-widest">{error}</span>
                  </div>
                )}

                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                  <div className="space-y-3">
                    <label className="text-[10px] font-black text-slate-100 uppercase tracking-widest px-1">Full Name*</label>
                    <input 
                      required
                      type="text" 
                      ...
                      className={`w-full bg-slate-950/50 border rounded-2xl px-6 py-4 text-sm text-white focus:outline-none transition-all placeholder:text-slate-800 ${error && formState.name.trim().length < 2 ? 'border-rose-500 bg-rose-500/10' : 'border-white/10 focus:border-emerald-500'}`}
                    />
                  </div>
                  <div className="space-y-3">
                    <label className="text-[10px] font-black text-slate-100 uppercase tracking-widest px-1">Email Protocol*</label>
                    <input 
                      required
                      type="email" 
                      ...
                      className={`w-full bg-slate-950/50 border rounded-2xl px-6 py-4 text-sm text-white focus:outline-none transition-all placeholder:text-slate-800 ${error && !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(formState.email) ? 'border-rose-500 bg-rose-500/10' : 'border-white/10 focus:border-emerald-500'}`}
                    />
                  </div>
                </div>

                <div className="space-y-3">
                  <label className="text-[10px] font-black text-slate-100 uppercase tracking-widest px-1">Subject*</label>
                  <input 
                    required
                    type="text" 
                    ...
                    className={`w-full bg-slate-950/50 border rounded-2xl px-6 py-4 text-sm text-white focus:outline-none transition-all placeholder:text-slate-800 ${error && formState.subject.trim().length < 3 ? 'border-rose-500 bg-rose-500/10' : 'border-white/10 focus:border-emerald-500'}`}
                  />
                </div>

                <div className="space-y-3">
                  <label className="text-[10px] font-black text-slate-100 uppercase tracking-widest px-1">Detailed Message*</label>
                  <textarea 
                    required
                    rows={4}
                    ...
                    className={`w-full bg-slate-950/50 border rounded-2xl px-6 py-4 text-sm text-white focus:outline-none transition-all placeholder:text-slate-800 resize-none ${error && formState.message.trim().length < 10 ? 'border-rose-500 bg-rose-500/10' : 'border-white/10 focus:border-emerald-500'}`}
                  />
                </div>
